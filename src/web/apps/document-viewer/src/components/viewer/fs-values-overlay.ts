...
export class FsValuesOverlay {
  ...
  constructor(private readonly _viewer: PdfViewer, private readonly _cache: FsValuesCache) {
  ...
  toggle(): boolean { this._visible ? this.hide() : this.show(); return this._visible; }

  show(): void { this._visible = true; this._renderAll(); }

  hide(): void { this._visible = false; this._clearAll(); }

  refresh(): void { this._visible ? this._renderAll() : this._clearAll(); }
