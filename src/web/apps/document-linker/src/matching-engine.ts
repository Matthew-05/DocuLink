...
import type { CharacterEntry, SearchPageIndex } from "@talliark/shared";
import type { LinkCreationRequest, LinkerPdf, LinkerRow, RowResult } from "./types/index.js";

interface PdfCache {
  id: string;
  name: string;
  pages: Map<number, LinkerPageCache>;
}

interface LinkerPageCache {
...
function buildLinkerPageCache(
  pages: Map<number, CharacterEntry[]>,
): Map<number, LinkerPageCache> {
    ...
    try {
      if (pdf.geometryBase64) {
        ...
        result.push({
          id: pdf.id,
          name: pdf.name,
          pages: buildLinkerPageCache(buildCharEntriesFromGeometry(geometry)),
        });
        continue;
      }
      ...
      console.warn("[Talliark] Skipping PDF with unreadable text content", pdf.name, error);
    }
  }
  return result;
}
...
export async function runMatching(
  ...
  for (const row of rows) {
    ...
    const normalizedTerms = row.keyValues.map((v) => normalizeLinkerQuery(v ?? "")).filter((t) => t.length > 0);
    ...
    for (let i = 0; i < row.keyValues.length; i++) {
      ...
      const normalized = normalizeLinkerQuery(rawValue);
      ...
      if (extractedText.length === 0) {
        console.warn(
          "[Talliark] Skipping linker link because matched rectangle extracted no text",
          {
