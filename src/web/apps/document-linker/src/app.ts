...
export function mountApp(root: HTMLElement): void {
  root.className = "linker-app";
  root.innerHTML = "";

  const indicatorWrap = document.createElement("div");
  indicatorWrap.className = "linker-app__indicator-wrap";
  root.appendChild(indicatorWrap);

  const stepContent = document.createElement("div");
  stepContent.className = "linker-app__step-content";
  ...
  initHostBridge({
    onLinkerReady(payload: LinkerReadyPayload) {
      selectionInfo = {
    ...
    async onLinkerDataLoaded(payload: LinkerDataLoadedPayload) {
      if (!step4) return;
      const view = step4;

      try {
        sendLinkerLog(
          `linker-data-loaded received rows=${payload.rows.length} pdfs=${payload.pdfs.length} outputs=${outputColNumbers.length}`,
        );
        sendLinkerLog(
          `linker inputs geometryPdfs=${payload.pdfs.filter((pdf) => !!pdf.geometryBase64).length} preparePdfs=${payload.pdfs.filter((pdf) => !pdf.geometryBase64 && !!pdf.base64).length} keyCounts=${payload.rows.map((row) => row.keyValues.length).join(",")} keyLengths=${payload.rows.map((row) => row.keyValues.map((value) => value.length).join("/")).join(",")}`,
        );
        ...
        sendLinkerLog(`matching complete requests=${requests.length} matched=${matched} unmatched=${unmatched}`);
        ...
        sendLinkerLog(`sending create-links count=${requests.length}`);
        sendCreateLinks(requests);
      } catch (error) {
        const message = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
        sendLinkerLog(`matching failed ${message}`);
        view.setPhase("error");
      }
    },
