...
const structureUrl = new URL("../src/financial-structure-decoder.ts", import.meta.url).href;
...
const { parseFinancialStructure } = await import(structureUrl) as typeof import("../src/financial-structure-decoder.ts");
...
test("parses canonical notes, continuation headers, and resolved citations", () => {
  const parsed = parseFinancialStructure({
    version: 1,
...
/**
 * The distinction financial analysis stands on: a compilation that has no items
...
test("rejects an unknown document class", () => {
  assert.throws(() => parseFinancialStructure({
    version: 1,
    coordinateSpace: "normalized",
    detectorVersion: "test",
