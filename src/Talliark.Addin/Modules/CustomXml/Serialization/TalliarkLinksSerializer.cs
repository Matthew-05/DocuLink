...
{
    public static class TalliarkLinksSerializer
    {
        ...
        public static IList<LinkedRectangle> FromXDocument(XDocument document)
        {
            ...
            if (root == null)
                throw new InvalidOperationException("Talliark links XML has no root element.");

            if (root.Name != TalliarkXml.LinksNs + TalliarkXml.LinksRootElementName)
                throw new InvalidOperationException(
                    "Root element must be {" + TalliarkXml.LinksNamespaceUri + "}" + TalliarkXml.LinksRootElementName + ".");

            ReadVersion(root);

            XElement linkedRectanglesElement = root.Element(TalliarkXml.LinksNs + TalliarkXml.LinkedRectanglesElementName);
            if (linkedRectanglesElement == null)
                throw new InvalidOperationException(
                    "Talliark links is missing required element '" + TalliarkXml.LinkedRectanglesElementName + "'.");

            return new List<LinkedRectangle>(
                linkedRectanglesElement
                    .Elements(TalliarkXml.LinksNs + TalliarkXml.LinkedRectangleElementName)
        ...
        public static XDocument ToXDocument(IEnumerable<LinkedRectangle> linkedRectangles)
        {
            ...
            var linkedRectanglesElement = new XElement(
                TalliarkXml.LinksNs + TalliarkXml.LinkedRectanglesElementName,
                rects.Select((rect, i) => SerializeLinkedRectangle(rect, i)));

            var root = new XElement(
                TalliarkXml.LinksNs + TalliarkXml.LinksRootElementName,
        ...
        private static void ReadVersion(XElement root)
        {
            ...
            if (versionAttribute == null)
                throw new InvalidOperationException("Talliark links root is missing required attribute 'version'.");
            ...
            if (!uint.TryParse(versionAttribute.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out uint fileVersion)
                ...
                throw new InvalidOperationException(
                    "Unsupported Talliark links version; expected " + TalliarkXml.SchemaVersion + ".");
        }
        ...
        private static LinkedRectangle ParseLinkedRectangle(XElement element, int index)
        {
            ...
            if (idAttribute == null || string.IsNullOrWhiteSpace(idAttribute.Value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " is missing required attribute 'id'.");

            XAttribute pdfIdAttribute = element.Attribute(TalliarkXml.PdfIdAttribute);
            if (pdfIdAttribute == null || string.IsNullOrWhiteSpace(pdfIdAttribute.Value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " is missing required attribute 'pdfId'.");

            XElement cellElement = element.Element(TalliarkXml.LinksNs + TalliarkXml.CellElementName);
            if (cellElement == null)
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " is missing required element '" + TalliarkXml.CellElementName + "'.");
            ...
            if (sheetAttribute == null || string.IsNullOrWhiteSpace(sheetAttribute.Value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Cell is missing required attribute 'sheet'.");
            if (addressAttribute == null || string.IsNullOrWhiteSpace(addressAttribute.Value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Cell is missing required attribute 'address'.");
            if (trackIndexAttribute == null
                ...
                || trackIndexValue <= 0)
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Cell is missing a valid positive 'trackIndex' attribute.");

            XElement rectElement = element.Element(TalliarkXml.LinksNs + TalliarkXml.RectElementName);
            if (rectElement == null)
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " is missing required element '" + TalliarkXml.RectElementName + "'.");
            ...
            if (pageAttr == null
                ...
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Rect is missing a valid non-negative 'page' attribute.");

            if (pageValue > int.MaxValue)
                throw new InvalidOperationException("Talliark links LinkedRectangle #" + index + " page index is too large.");
        ...
        private static RectangleCoordinateSpace ParseCoordinateSpace(XElement rectElement, int index)
        {
            ...
            if (spaceAttr == null || string.IsNullOrWhiteSpace(spaceAttr.Value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Rect is missing required attribute 'coordinateSpace'.");
            ...
            throw new InvalidOperationException(
                "Talliark links LinkedRectangle #" + index + " has unsupported coordinateSpace '" + value + "'.");
        }
        ...
        private static double ReadRequiredDouble(XElement element, string attributeName, int index)
        {
            ...
            if (attribute == null)
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Rect is missing required attribute '" + attributeName + "'.");

            if (!double.TryParse(attribute.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out double value))
                throw new InvalidOperationException(
                    "Talliark links LinkedRectangle #" + index + " Rect attribute '" + attributeName + "' is not a valid number.");
        ...
        private static XElement SerializeLinkedRectangle(LinkedRectangle linkedRect, int index)
        {
            ...
            var element = new XElement(
                TalliarkXml.LinksNs + TalliarkXml.LinkedRectangleElementName,
                new XAttribute(IdAttribute, linkedRect.Id),
                new XAttribute(TalliarkXml.PdfIdAttribute, linkedRect.PdfId),
                new XAttribute(LinkTypeAttribute, SerializeLinkType(linkedRect.LinkType)),
                new XElement(
                    TalliarkXml.LinksNs + TalliarkXml.CellElementName,
        ...
        {
            XElement gridElement = element.Element(TalliarkXml.LinksNs + TableGridElement);
            if (gridElement == null)
            ...
            XElement gridElement, string elementName, int rectangleIndex)
        {
            ...
            foreach (XElement boundary in gridElement.Elements(TalliarkXml.LinksNs + elementName))
            {
                double position = ReadRequiredDouble(boundary, PositionAttribute, rectangleIndex);
                if (position <= 0 || position >= 1)
                    throw new InvalidOperationException(
                        $"Talliark links LinkedRectangle #{rectangleIndex} has a {elementName} outside (0, 1)." );
                result.Add(position);
            }
        ...
        private static XElement SerializeTableGrid(TableGrid tableGrid)
        {
            ...
            var element = new XElement(TalliarkXml.LinksNs + TableGridElement);
            ...
            XElement parent, string elementName, IEnumerable<double> boundaries)
        {
            foreach (double position in NormalizeBoundaries(boundaries))
            {
                parent.Add(new XElement(
                    TalliarkXml.LinksNs + elementName,
        ...
        private static XElement SerializeRect(PdfRectangle rect, string linkedRectId)
        {
            ...
            return new XElement(
                TalliarkXml.LinksNs + TalliarkXml.RectElementName,
