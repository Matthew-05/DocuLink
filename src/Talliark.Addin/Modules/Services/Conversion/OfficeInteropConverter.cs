...
namespace Talliark.Addin.Modules.Services.Conversion
{
    ...
    /// deliberate: Talliark only references the Excel PIA, and adding Word/PowerPoint/
    ...
    ///
    /// Excel is deliberately absent. It was the extreme case of adoption — Talliark
    ...
    internal sealed class OfficeInteropConverter : IDisposable
    {
        ...
        public OfficeInteropConverter()
        {
            ...
            {
                IsBackground = true,
                Name = "Talliark Office Automation",
            };
        ...
        private void RunMessageLoop()
        {
            ...
            {
                TalliarkLog.Trace($"Office automation thread stopped: {ex}");
            }
            finally
            {
                ...
                catch (Exception ex) { TalliarkLog.Trace($"Office automation window dispose failed: {ex.Message}"); }
            }
        }
        ...
        private bool Post(Action work, Action onCannotPost)
        {
            try
            {
                ...
                {
                    TalliarkLog.Trace("Office automation thread did not start within the timeout.");
                    onCannotPost?.Invoke();
                    return false;
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                TalliarkLog.Trace($"Could not post to the Office automation thread: {ex.Message}");
                onCannotPost?.Invoke();
                return false;
            }
        }
        ...
        private void ConvertWithWord(string sourcePath, string outputPdfPath)
        {
            ...
            finally
            {
                if ((object)document != null)
                {
                    ...
                    catch (Exception ex) { TalliarkLog.Trace($"Could not close the converted document: {ex.Message}"); }
                }
        ...
        private void ConvertWithPowerPoint(string sourcePath, string outputPdfPath)
        {
            ...
            finally
            {
                if ((object)presentation != null)
                {
                    ...
                    catch (Exception ex) { TalliarkLog.Trace($"Could not close the converted presentation: {ex.Message}"); }
                }
        ...
        private void ConvertMessageToHtml(string sourcePath, string outputHtmlPath)
        {
            ...
            finally
            {
                if ((object)item != null)
                {
                    ...
                    catch (Exception ex) { TalliarkLog.Trace($"Could not close the converted message: {ex.Message}"); }
                }
        ...
        private static ApplicationHandle AttachToRunningApplication(string progId)
        {
            try
            {
                ...
                TalliarkLog.Trace($"Attached to a running '{progId}'; it will not be quit on shutdown.");
            ...
            {
                TalliarkLog.Trace($"Could not check for a running '{progId}': {ex.Message}");
                return null;
            }
        }
        ...
        private static ApplicationHandle LaunchApplication(ConversionEngine engine, Type type)
        {
            try
            {
                ...
                TalliarkLog.Trace($"Started our own {GetApplicationName(engine)}; it will be quit on shutdown.");
                return handle;
            }
            catch (Exception ex)
            {
        ...
        private static void Configure(ApplicationHandle handle, string property, object value)
        {
            ...
            {
                TalliarkLog.Trace($"Could not set '{property}': {ex.Message}");
            }
        }
        ...
        private static void Release(object comObject)
        {
            ...
            {
                TalliarkLog.Trace($"Could not release a COM object: {ex.Message}");
            }
        }
        ...
        private void ShutdownApplications()
        {
            foreach (var pair in _applications)
            {
                ...
                if (mayQuit)
                {
                    ...
                    {
                        TalliarkLog.Trace($"Could not quit {GetApplicationName(pair.Key)}: {ex.Message}");
                    }
                }
        ...
        public async Task ShutdownAsync()
        {
            ...
            {
                TalliarkLog.Trace($"Office automation shutdown failed: {ex}");
            }
            ...
            using (var shutdownComplete = new ManualResetEventSlim(false))
            {
                bool posted = Post(
                    () =>
                    {
                        ...
                        catch (Exception ex) { TalliarkLog.Trace($"OfficeInteropConverter dispose failed: {ex}"); }
                        finally
                        {
        ...
        private void StopThread()
        {
            ...
            {
                TalliarkLog.Trace($"Office automation thread did not stop cleanly: {ex}");
            }
            finally
            {
