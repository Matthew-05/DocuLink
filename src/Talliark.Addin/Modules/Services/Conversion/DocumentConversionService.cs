...
namespace Talliark.Addin.Modules.Services.Conversion
{
    ...
    internal sealed class DocumentConversionService
    {
                ...
                using (var html = new HtmlToPdfConverter())
                {
                    for (int i = 0; i < total; i++)
                    {
                        ...
                        try
                        {
                            ...
                            using (TalliarkLog.Time($"Converting '{displayName}' ({current} of {total})"))
                        ...
                        catch (Exception ex)
                        {
                            ...
                            TalliarkLog.Trace($"Conversion failed for '{displayName}': {ex}");
                        }
                    }
                }
            }
        ...
        private static void DeleteSidecarFolder(string htmlPath)
        {
            ...
            {
                TalliarkLog.Trace($"Could not delete Outlook sidecar folder for '{htmlPath}': {ex.Message}");
            }
        }
    }
}
