...
namespace Talliark.Addin.Modules.Services
{
    ...
    {
        private const string ReleasesApiUrl = "https://api.github.com/repos/Matthew-05/Talliark/releases";
        private const string ReleasesPageUrl = ReleasesApiUrl + "?per_page=100&page=";
        private const string ReleasesUrl = "https://github.com/Matthew-05/Talliark/releases";
        ...
        private static async Task<UpdateCheckResult> CheckCoreAsync(bool isDevBuild)
        {
            ...
            {
                client.DefaultRequestHeaders.UserAgent.ParseAdd("Talliark-Addin/" + AppVersion.Current);
        ...
        internal static async Task<string> DownloadAsync(string url, string version, IProgress<int> progress, CancellationToken cancellationToken = default)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    var localPath = Path.Combine(Path.GetTempPath(), $"Talliark-Setup-{version}.msi");
