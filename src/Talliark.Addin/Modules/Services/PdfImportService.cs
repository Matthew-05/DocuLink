...
namespace Talliark.Addin.Modules.Services
{
    internal sealed class PdfImportService
    {
            ...
            IProgressReporter progress)
        {
            ...
            TalliarkLog.Trace($"Importing {total} PDF(s) from disk.");
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    TalliarkLog.Trace($"Could not import '{fileName}' from '{request.Path}': {ex}");
                    progress?.Report("Could not import document", $"{fileName} ({current} of {total})", current, total);
                }
            }

            TalliarkLog.Trace($"Import from disk finished: {result.AddedIds.Count} added, {result.Errors.Count} failed.");
            return result;
        }
            ...
            IProgressReporter progress)
        {
            ...
            TalliarkLog.Trace($"Importing {total} PDF(s) supplied as base-64.");
            ...
            for (int i = 0; i < total; i++)
            {
                ...
                {
                    result.Errors.Add($"{fileName}: {ex.Message}");
                    TalliarkLog.Trace($"Could not import '{fileName}' from base-64: {ex}");
                    progress?.Report("Could not import document", $"{fileName} ({current} of {total})", current, total);
                }
            }

            TalliarkLog.Trace($"Import from base-64 finished: {result.AddedIds.Count} added, {result.Errors.Count} failed.");
            return result;
        }
